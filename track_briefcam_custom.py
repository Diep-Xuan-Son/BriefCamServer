...
def run(
	...
	BC = BriefCam(name_vid=name_vid, path_result=PATH_RESULT_ABSOLUTE)
	...
	for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
		...
		percentComplete = (add_percent + (frame_idx+1)*0.75*scale_percent/total_frame)*100
		with open(os.path.join(PATH_LOG_ABSOLUTE,'percent.txt'), 'w') as f:
			f.write(f"{percentComplete:.2f}")
		# percentComplete = (frame_idx+1)*0.75/total_frame
	...
	BC.dict_data["information"]["fps"] = fpsss
	...
	BC.brief(BC.dict_data['data'], BC.dict_data['information'], scale_percent, add_percent, PATH_LOG_ABSOLUTE)
	#----------------------------------
	# Print results
	...
	return os.path.join(PATH_RESULT,f'{BC.name_vid}_brief.mp4')
...
def track_briefcam(inputts, objectt, ipaddress):
...
# def check_complete_percent():
# 	global percentComplete
# # 	if check_complete_brief() > 0.75:
# # 		percentComplete = check_complete_brief()
